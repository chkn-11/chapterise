...
- Speech near each marker (the same ±8-second preview window) appears under its title. Adjusting the marker refreshes the displayed text. Segment timestamps are approximate, so displayed sentences can extend beyond the preview window.
- Search finds literal, case-insensitive words or phrases within transcript passages throughout the recording. Select a result to play that passage and fill the manual chapter time, then add a missing break if appropriate. The first 100 matching passages are shown; narrow the query for more specific results.
