    ...
    def test_review_scan_and_export(self):
        ...
        with tempfile.TemporaryDirectory(prefix='chapterise-browser-') as profile:
            try:
                ...
                evaluate("document.querySelector('#rows tr:nth-child(2) button').click()")
                ...
                evaluate("document.getElementById('manual-time').value = '00:00:11.000'; document.getElementById('manual-title').value = 'Missing chapter'; document.getElementById('preview-manual').click()")
                self.assertAlmostEqual(evaluate("previewEnd"), 19, places=3)
                ...
                evaluate("document.querySelector('.transcript-result').click()")
                self.assertEqual(evaluate("document.getElementById('manual-time').value"), '00:00:09.000')
                self.assertAlmostEqual(evaluate("previewEnd"), 10.5)
                ...
                self.assertAlmostEqual(evaluate("rows[1].start - rows[1].originalStart"), 1.25, places=3)
                evaluate("document.querySelector('#rows tr:nth-child(2) button').click()")
                self.assertAlmostEqual(evaluate("previewEnd"), adjusted + 8, places=3)
